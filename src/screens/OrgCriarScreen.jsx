...
export default function OrgCriarScreen() {
  ...
  return (
    <div className="screen">
      <div className="header"><button className="btn-back" onClick={() => navigate('org-escolha')}><Icon name="chevron-left" size={20} /></button><span className="header-title">Criar Organização</span></div>
      <div className="content">
        {erro && <div className="auth-error">{erro}</div>}
        <div className="campo"><label>Nome da organização</label><input type="text" value={nome} onChange={e => setNome(e.target.value)} onKeyDown={e => e.key === 'Enter' && criar()} placeholder="Ex: Igreja Aliança Vizela" /></div>
        <button className="btn-primary btn-accent" onClick={criar} disabled={loading} style={{ marginTop: '8px' }}>{loading ? 'A criar...' : 'Criar Organização'}</button>
      </div>
    </div>
  )
}
