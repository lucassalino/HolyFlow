...
export default function MomentoScreen() {
  ...
  const onSelectChange = (v) => {
    setSel(v)
    if (v === 'Louvor' && !musicas.length) setMusicas([''])
    if (v === 'Avisos' && !avisos.length) setAvisos([''])
  }
  ...
  const showMusicas = sel === 'Louvor'
  const showAvisos = sel === 'Avisos'
  const showPalavra = sel === 'Palavra' || sel === 'Ministração'
      ...
      <div className="content">
        ...
        {sel === 'outro' && (
          <div className="campo">
            ...
            <input type="text" value={custom} onChange={e => setCustom(e.target.value)} placeholder="Escreve o nome do momento..." autoFocus />
          </div>
        )}
        ...
        <div className="campo">
          ...
          <div className="tipo-selector">
            {['pessoa', 'video', 'projecao'].map(t => (
              <button key={t} className={`tipo-btn${tipo === t ? ' selected' : ''}`} onClick={() => setTipo(t)}>
                {t === 'pessoa' ? 'Pessoa' : t === 'video' ? 'Vídeo' : 'Projeção'}
              </button>
            ))}
          </div>
        </div>
        ...
        <div className="campo">
          ...
          <input type="text" value={duracao} onChange={e => setDuracao(e.target.value)} placeholder="Ex: 5 min, 25 min..." />
        </div>
        <div className="campo">
