    ...
    <div className="screen">
      ...
      <div className="content">
        ...
        {momentos.length === 0 ? (
          ...
          momentos.map((m, i) => (
            <div key={i} className="momento-item" draggable data-idx={i} onClick={() => editarMomento(i)}
              onDragStart={e => onDragStart(e, i)} onDragOver={e => onDragOver(e, i)} onDragLeave={onDragLeave} onDrop={e => onDrop(e, i)} onDragEnd={onDragEnd}>
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                ...
                {m.musicas?.filter(x => x).length > 0 && <div className="momento-sub">🎵 {m.musicas.filter(x => x).join(' · ')}</div>}
                {m.avisos?.filter(x => x).length > 0 && <div className="momento-sub">📢 {m.avisos.filter(x => x).length} aviso{m.avisos.filter(x => x).length > 1 ? 's' : ''}</div>}
                {m.obs && <div className="momento-sub">{m.obs}</div>}
              </div>
              {renderBadge(m)}
              <button className="btn-del" onClick={e => removerMomento(e, i)}><Icon name="trash" size={16} /></button>
            </div>
          ))
        )}
      </div>
